...
public class TPSCameraControllerEditor : Editor
{
    ...
    public void CameraPositionAdjustmentVariables(CamPivotController target)
    {
        if (CameraPositionAdjustment)
        {
            ...
            serializedObject.FindProperty("UnderAngleLimit").floatValue = EditorGUILayout.Slider("  UnderAngleLimit",target.UnderAngleLimit,0,60);
            ...
            serializedObject.FindProperty("MinRotation").floatValue = EditorGUILayout.Slider("  MinRotation",target.MinRotation,-180,target.MaxRotation);
            serializedObject.FindProperty("MaxRotation").floatValue = EditorGUILayout.Slider("  MaxRotation",target.MaxRotation,target.MinRotation,180);
        }
    }
