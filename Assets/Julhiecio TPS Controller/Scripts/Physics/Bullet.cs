...
public class Bullet : MonoBehaviour
{
	...
	void OnCollisionEnter(Collision col)
	{
		if (col.gameObject.tag != "Bullet" && col.gameObject.tag != "Player")
		{
			...
			//Cause Damage if enemy
			if (col.gameObject.tag == "Enemy")
            {
				int InstanceID = col.gameObject.GetComponent<Unit>().InstanceID;
				GameManager.Instance.GetAttack(InstanceID,ShotWeapon.ATK);
				//触发技能效果
				if(BulletEffect!=null)
                {
					GameObject effect = GameObject.Instantiate(BulletEffect, col.transform);
